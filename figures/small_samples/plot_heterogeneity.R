plot_heterogeneity <- function(test_type=1, allsimudat=NA) {
    ...
    robutness_plot <- function(data){
        ...
        p1 <- plot_qq_p(
                 subset(data_positive_z, methods %in% homogeneity_methods &
                                             nSubjects==20 & nStudies==25+25*(data$glm[1] > 1) &
                                             withinVariation!=1 & Between==1),
                formula=Between~methods, "Heteroscedasticity", short=TRUE, lim=10) + 
        ...
        p2 <- plot_qq_p(
                subset(data_positive_z, methods %in% homogeneity_methods &
                                             nSubjects==20 & nStudies==25+25*(data$glm[1] > 1) &
                                             withinVariation==1 & Between==1),
                formula=Between~methods, "Homoscedasticity", short=TRUE, lim=10) + 
            theme(legend.position="right") + 
            scale_fill_manual(values=cbfPalette) + 
    ...
    # Save to pdf
    pdf(paste("robustness_heterogeneity.pdf", sep=""))
    print(p)
    dev.off()
